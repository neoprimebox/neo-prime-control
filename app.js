const K={
  ...
  suppliers:"npc_v13_8_suppliers", messages:"npc_v13_8_messages", settings:"npc_v13_8_settings", purchaseAccounts:"npc_v14_purchase_accounts", agentDecisions:"npc_v20_agent_decisions", agentLog:"npc_v20_agent_log", agentConfig:"npc_v20_agent_config", amazonIntegration:"npc_v21_amazon_integration", amazonCatalog:"npc_v21_amazon_catalog", amazonLog:"npc_v21_amazon_log", seeded:"npc_v13_8_seeded"
};
const STORE="Neo Prime Box";
const APP_VERSION="20.0-beta";
...
function customerNotesToDb(x){
  const extras={cpf:x.cpf||"", cep:x.cep||"", address:x.address||"", number:x.number||"", complement:x.complement||"", district:x.district||"", city:x.city||"", uf:x.uf||"", notes:x.notes||""};
...
function orderNotesToDb(x){
  const extras={customerCpf:x.customerCpf||"", purchaseMarketplace:x.purchaseMarketplace||"", purchaseAccount:x.purchaseAccount||"", marketplaceOrderId:x.marketplaceOrderId||"", agentDecision:x.agentDecision||"", notes:x.notes||""};
  const hasExtras=extras.customerCpf || extras.purchaseMarketplace || extras.purchaseAccount || extras.marketplaceOrderId || extras.agentDecision;
  return hasExtras?`NPC_ORDER_EXTRA:${JSON.stringify(extras)}`:(x.notes||null);
}
function orderNotesFromDb(v){
  const s=String(v||"");
  if(!s.startsWith("NPC_ORDER_EXTRA:")) return {notes:s, customerCpf:"", purchaseMarketplace:"", purchaseAccount:"", marketplaceOrderId:"", agentDecision:""};
  try{return JSON.parse(s.slice("NPC_ORDER_EXTRA:".length));}catch(e){return {notes:s, customerCpf:"", purchaseMarketplace:"", purchaseAccount:"", marketplaceOrderId:"", agentDecision:""};}
}
...
function dbToJs(name,x){
  ...
  if(name==="customers"){const extra=customerNotesFromDb(x.observacoes);return {id:x.id,name:x.nome,phone:x.telefone||"",email:x.email||"",cpf:extra.cpf||"",cep:extra.cep||"",address:extra.address||"",number:extra.number||"",complement:extra.complement||"",district:extra.district||"",city:extra.city||"",uf:extra.uf||"",notes:extra.notes||"",createdAt:x.data_criacao,updatedAt:x.data_atualizacao};}
  ...
  if(name==="orders"){const extra=orderNotesFromDb(x.observacoes);return {id:x.id,orderDate:x.data_pedido||"",amazonOrderId:x.numero_pedido_amazon||"",productId:x.produto_id||"",productName:x.nome_produto||"",customerId:x.cliente_id||"",customerName:x.nome_cliente||"",customerPhone:x.telefone_cliente||"",customerCpf:extra.customerCpf||"",customerCep:x.cep_cliente||"",customerAddress:x.endereco_cliente||"",customerNumber:x.numero_cliente||"",customerComplement:x.complemento_cliente||"",customerDistrict:x.bairro_cliente||"",customerCity:x.cidade_cliente||"",customerUf:x.uf_cliente||"",supplierId:x.fornecedor_id||"",purchaseMarketplace:extra.purchaseMarketplace||"",purchaseAccount:extra.purchaseAccount||"",marketplaceOrderId:extra.marketplaceOrderId||"",agentDecision:extra.agentDecision||"",buyLink:x.link_compra||"",status:x.status||"Venda realizada Amazon",salePrice:num(x.preco_venda),saleShipping:num(x.frete_venda),quantity:num(x.quantidade)||1,totalRevenue:num(x.receita_total),buyPrice:num(x.preco_compra),buyShipping:num(x.frete_compra),buyDiscount:num(x.desconto_compra),totalSupplier:num(x.custo_total_fornecedor),supplierQuantity:num(x.quantidade_fornecedor)||1,amazonFees:num(x.taxas_amazon),netProfit:num(x.lucro_liquido),hasNetProfit:!!x.tem_lucro_liquido,netProfitSource:x.origem_lucro_liquido||"",trackingCode:x.codigo_rastreio||"",trackingSent:x.rastreio_enviado||"Não",messageTemplateId:x.mensagem_id||"",notes:extra.notes||"",createdAt:x.data_criacao,updatedAt:x.data_atualizacao};}
  return null;
}
...
function openView(view){
  ...
  if(previousView!==view && $("globalSearch")){
    $("globalSearch").value="";
    orderPage=1;
    lastProductAutoFillQuery="";
  }
...
function applySidebarState(){
  const collapsed=localStorage.getItem("npc_sidebar_collapsed")==="1";
  document.body.classList.toggle("sidebar-collapsed",collapsed);
  if($("sidebarToggle")) $("sidebarToggle").textContent=collapsed?"☰ Menu":"☰";
}
function toggleSidebar(){
  const collapsed=!document.body.classList.contains("sidebar-collapsed");
  localStorage.setItem("npc_sidebar_collapsed",collapsed?"1":"0");
  applySidebarState();
  setTimeout(()=>{try{renderReports();}catch(e){}},120);
}
...
function filteredOrders(){
  ...
  return read(K.orders).filter(o=>{
    const blob=`${o.amazonOrderId} ${o.productName} ${o.customerName} ${o.customerPhone||""} ${o.customerCpf||""} ${o.customerCep||""} ${o.customerCity||""} ${o.customerUf||""} ${supplierName(o.supplierId)} ${orderMarketplace(o)} ${o.purchaseMarketplace||""} ${o.purchaseAccount||""} ${o.status} ${o.trackingCode||""}`;
...
$("orderCustomerId").onchange=()=>{const c=read(K.customers).find(x=>x.id===$("orderCustomerId").value);if(!c)return;["Name","Phone","Cpf","Cep","Address","Number","Complement","District","City","Uf"].forEach(f=>{$("customer"+f).value=c[f.toLowerCase()]||c[f.charAt(0).toLowerCase()+f.slice(1)]||""});};
...
function upsertCustomer(o){
  ...
  const c={id:ix>=0?arr[ix].id:uuid(),name:o.customerName,phone:o.customerPhone,cpf:o.customerCpf||"",cep:o.customerCep,address:o.customerAddress,number:o.customerNumber,complement:o.customerComplement,district:o.customerDistrict,city:o.customerCity,uf:o.customerUf};
...
function orderFromForm(){
  ...
  return {id:$("orderId").value||uuid(),orderDate:$("orderDate").value,amazonOrderId:$("amazonOrderId").value,productId:$("orderProductId").value||p?.id||old.productId||"",productName,customerId:$("orderCustomerId").value,customerName:$("customerName").value,customerPhone:$("customerPhone").value,customerCpf:$("customerCpf")?.value||"",customerCep:$("customerCep").value,customerAddress:$("customerAddress").value,customerNumber:$("customerNumber").value,customerComplement:$("customerComplement").value,customerDistrict:$("customerDistrict").value,customerCity:$("customerCity").value,customerUf:$("customerUf").value,supplierId:$("orderSupplierId").value,purchaseMarketplace:$("purchaseMarketplace")?.value||"",marketplaceOrderId:$("marketplaceOrderId")?.value||"",purchaseAccount:$("purchaseAccount")?.value||"",buyLink:$("buyLink").value,status:$("orderStatus").value,salePrice:num($("salePrice").value),quantity:Math.max(1,parseInt($("orderQuantity")?.value||"1",10)||1),saleShipping:num($("saleShipping").value),buyPrice:num($("buyPrice").value),buyShipping:num($("buyShipping").value),buyDiscount:num($("buyDiscount").value),amazonFees:num($("amazonFees").value),trackingCode:$("trackingCode").value,trackingSent:$("trackingSent").value,messageTemplateId:$("messageTemplateId").value,notes:$("orderNotes").value,updatedAt:new Date().toISOString()};
}
...
function clearOrder(){ $("orderForm").reset(); $("orderId").value=""; $("orderFormTitle").textContent="Novo pedido Amazon"; $("orderDate").value=today(); ["saleShipping","buyShipping","buyDiscount","amazonFees"].forEach(id=>$(id).value=0); if($("orderQuantity")) $("orderQuantity").value=1; if($("purchaseAccount")) $("purchaseAccount").value=""; if($("purchaseMarketplace")) $("purchaseMarketplace").value=""; if($("marketplaceOrderId")) $("marketplaceOrderId").value="";}
...
function setFormValues(map){
  Object.entries(map).forEach(([k,v])=>{
    ...
    if(!el) return;
    el.value=(v ?? "");
    // Força selects a manterem o valor mesmo depois de render/recriação das opções.
...
function fillOrderForm(o,{scroll=true}={}){
  if(!o) return;
  o=resolveOrderProductSelection(o);
  syncResolvedProductBackToOrder(o);
  renderSelects();
  const resolvedProduct=findProductForOrder(o);
  ensureOrderProductOption(resolvedProduct);
  ...
  const map={orderId:o.id,orderDate:o.orderDate,amazonOrderId:o.amazonOrderId,orderProductId:productId,orderCustomerId:o.customerId,customerName:o.customerName,customerPhone:o.customerPhone,customerCpf:o.customerCpf||"",customerCep:o.customerCep,customerAddress:o.customerAddress,customerNumber:o.customerNumber,customerComplement:o.customerComplement,customerDistrict:o.customerDistrict,customerCity:o.customerCity,customerUf:o.customerUf,orderSupplierId:supplierId,purchaseMarketplace:o.purchaseMarketplace||"",marketplaceOrderId:o.marketplaceOrderId||"",purchaseAccount:o.purchaseAccount||"",buyLink:o.buyLink,orderStatus:o.status,salePrice:o.salePrice,orderQuantity:lineQuantity(o),saleShipping:o.saleShipping,buyPrice:o.buyPrice,buyShipping:o.buyShipping,buyDiscount:o.buyDiscount,amazonFees:o.amazonFees,trackingCode:o.trackingCode,trackingSent:o.trackingSent,messageTemplateId:o.messageTemplateId,orderNotes:o.notes};
  setFormValues(map);
  if($('orderProductId')) $('orderProductId').value=productId;
  if($('orderSupplierId')) $('orderSupplierId').value=supplierId;
  if($("orderFormTitle")) $("orderFormTitle").textContent=`Editar pedido Amazon${productName?` · ${productName}`:""}`;
  updateOrderCalcPreview();
  if(scroll) document.querySelector(".formPanel")?.scrollIntoView({behavior:"smooth",block:"start"});
}
function editOrder(id){
  const original=read(K.orders).find(x=>x.id===id);
  if(!original) return;
  if(activeView()!=="orders"){
    openView("orders");
    setTimeout(()=>fillOrderForm(original,{scroll:true}),0);
  }else{
    fillOrderForm(original,{scroll:true});
  }
}
...
function reportMonthRows(){
  return Object.values(groupBy(read(K.orders),o=>monthKey(o.orderDate))).map(arr=>({
    month:monthKey(arr[0]?.orderDate),
    orders:arr.length,
    qty:arr.reduce((s,o)=>s+lineQuantity(o),0),
    rev:arr.reduce((s,o)=>s+revenue(o),0),
    supplier:arr.reduce((s,o)=>s+supplierOrderCost(o),0),
    amazon:arr.reduce((s,o)=>s+num(o.amazonFees),0),
    profit:arr.reduce((s,o)=>s+profit(o),0)
  })).sort((a,b)=>String(a.month).localeCompare(String(b.month)));
}
function reportSelectedOrders(){
  const mode=$("reportMode")?.value||"month";
  const day=$("reportDay")?.value||today();
  const month=$("reportMonth")?.value||today().slice(0,7);
  const all=read(K.orders);
  if(mode==="day") return all.filter(o=>String(o.orderDate||"").slice(0,10)===day);
  if(mode==="month") return all.filter(o=>String(o.orderDate||"").slice(0,7)===month);
  return all;
}
function productStatsFromOrders(orders){
  const m={};
  (orders||[]).forEach(o=>{
    const k=o.productName||productName(o.productId)||"Produto sem nome";
    m[k]??={name:k,qty:0,orders:0,rev:0,supplier:0,amazon:0,profit:0};
    m[k].qty+=lineQuantity(o); m[k].orders++; m[k].rev+=revenue(o); m[k].supplier+=supplierOrderCost(o); m[k].amazon+=num(o.amazonFees); m[k].profit+=profit(o);
  });
  return Object.values(m).sort((a,b)=>b.qty-a.qty || b.rev-a.rev || b.profit-a.profit);
}
function drawMonthlyBars(rows){
  const c=$("monthlyBarsChart"); if(!c || !c.getContext) return;
  const ctx=c.getContext("2d"), w=c.width, h=c.height;
  ctx.clearRect(0,0,w,h);
  const padL=72,padR=24,padT=28,padB=64;
  const chartW=w-padL-padR, chartH=h-padT-padB;
  const vals=(rows||[]).map(r=>r.rev);
  const max=Math.max(1,...vals);
  ctx.strokeStyle="rgba(255,255,255,.16)"; ctx.lineWidth=1;
  ctx.fillStyle="#98a6c6"; ctx.font="12px Segoe UI, Arial";
  for(let i=0;i<=4;i++){
    const y=padT+chartH-(chartH*i/4);
    ctx.beginPath(); ctx.moveTo(padL,y); ctx.lineTo(w-padR,y); ctx.stroke();
    ctx.fillText(brl(max*i/4).replace('R$',''), 8, y+4);
  }
  if(!rows.length){ctx.fillText("Sem dados para gerar gráfico.", padL, padT+40); return;}
  const gap=14;
  const bw=Math.max(22,(chartW-gap*(rows.length+1))/rows.length);
  rows.forEach((r,i)=>{
    const x=padL+gap+i*(bw+gap);
    const bh=(r.rev/max)*chartH;
    const y=padT+chartH-bh;
    const grad=ctx.createLinearGradient(0,y,0,padT+chartH);
    grad.addColorStop(0,"#e647d4"); grad.addColorStop(.55,"#7c2cff"); grad.addColorStop(1,"#2563ff");
    ctx.fillStyle=grad;
    ctx.beginPath(); ctx.roundRect(x,y,bw,bh,8); ctx.fill();
    ctx.fillStyle="#f8fbff"; ctx.font="bold 12px Segoe UI, Arial";
    ctx.fillText(brl(r.rev).replace('R$',''), x, Math.max(14,y-7));
    ctx.save(); ctx.translate(x+bw/2,h-22); ctx.rotate(-Math.PI/7);
    ctx.fillStyle="#98a6c6"; ctx.font="12px Segoe UI, Arial"; ctx.textAlign="center";
    ctx.fillText(monthLabel(r.month),0,0); ctx.restore();
  });
}

function orderMarketplace(o){
  const raw=[o.marketplace,o.salesChannel,o.channel,o.canal,o.origem,o.source,o.notes,supplierName(o.supplierId),o.status].join(" ").toLowerCase();
  if(/tik\s*tok|tiktok/.test(raw)) return "TikTok";
  if(/shopee/.test(raw)) return "Shopee";
  if(/mercado\s*livre|mercadolivre|meli/.test(raw)) return "Mercado Livre";
  if(/amazon/.test(raw)) return "Amazon";
  return "Não informado";
}
function genericStatsFromOrders(orders, keyFn, label="name"){
  const m={};
  (orders||[]).forEach(o=>{
    const k=keyFn(o)||"Não informado";
    m[k]??={name:k,orders:0,qty:0,rev:0,supplier:0,amazon:0,profit:0,last:""};
    m[k].orders++; m[k].qty+=lineQuantity(o); m[k].rev+=revenue(o); m[k].supplier+=supplierOrderCost(o); m[k].amazon+=num(o.amazonFees); m[k].profit+=profit(o);
    if(String(o.orderDate||"")>String(m[k].last||"")) m[k].last=o.orderDate||"";
  });
  return Object.values(m).sort((a,b)=>b.rev-a.rev || b.profit-a.profit || b.orders-a.orders);
}
function currentAndPreviousMonthRows(){
  const now=today().slice(0,7);
  const d=new Date(`${now}-01T00:00:00`); d.setMonth(d.getMonth()-1);
  const prev=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`;
  const rows=reportMonthRows();
  const empty=m=>({month:m,orders:0,qty:0,rev:0,supplier:0,amazon:0,profit:0});
  return [rows.find(r=>r.month===now)||empty(now), rows.find(r=>r.month===prev)||empty(prev)];
}

function renderReports(){
  const mode=$("reportMode")?.value||"month";
  const selected=reportSelectedOrders();
  const months=reportMonthRows();
  const best=months.slice().sort((a,b)=>b.rev-a.rev)[0];
  const rev=selected.reduce((s,o)=>s+revenue(o),0), prof=selected.reduce((s,o)=>s+profit(o),0), qty=selected.reduce((s,o)=>s+lineQuantity(o),0);
  if($("reportRevenueKpi")) $("reportRevenueKpi").textContent=brl(rev);
  if($("reportProfitKpi")) $("reportProfitKpi").textContent=brl(prof);
  if($("reportOrdersKpi")) $("reportOrdersKpi").textContent=selected.length;
  if($("reportQtyKpi")) $("reportQtyKpi").textContent=qty;
  if($("reportBestMonthKpi")) $("reportBestMonthKpi").textContent=best?monthLabel(best.month):"-";
  if($("reportBestMonthSub")) $("reportBestMonthSub").textContent=best?brl(best.rev):brl(0);
  if($("reportPeriodTitle")){
    const title=mode==="day"?`Produtos vendidos no dia ${($("reportDay")?.value||today()).split('-').reverse().join('/')}`:mode==="month"?`Produtos vendidos no mês ${monthLabel($("reportMonth")?.value||today().slice(0,7))}`:"Produtos vendidos no total";
    $("reportPeriodTitle").textContent=title;
  }
  const ps=productStatsFromOrders(selected);
  if($("reportProductsTable")) $("reportProductsTable").innerHTML=ps.slice(0,10).map((p,i)=>`<tr><td>${i+1}</td><td><b>${esc(p.name)}</b></td><td>${p.qty}</td><td>${p.orders}</td><td>${brl(p.rev)}</td><td>${brl(p.supplier)}</td><td>${brl(p.amazon)}</td><td class="success">${brl(p.profit)}</td><td>${p.rev?(p.profit/p.rev*100).toFixed(1):"0.0"}%</td></tr>`).join("") || `<tr><td colspan="9"><small>Sem produtos para o período escolhido.</small></td></tr>`;
  if($("reportMonthlyTable")) $("reportMonthlyTable").innerHTML=months.slice().reverse().map(m=>`<tr><td><b>${monthLabel(m.month)}</b></td><td>${m.orders}</td><td>${m.qty}</td><td>${brl(m.rev)}</td><td>${brl(m.supplier)}</td><td>${brl(m.amazon)}</td><td class="success">${brl(m.profit)}</td><td>${m.rev?(m.profit/m.rev*100).toFixed(1):"0.0"}%</td></tr>`).join("") || `<tr><td colspan="8"><small>Sem movimento mensal.</small></td></tr>`;
  if($("reportTicketKpi")) $("reportTicketKpi").textContent=brl(selected.length?rev/selected.length:0);
  const [cur,prev]=currentAndPreviousMonthRows();
  const diff=prev.rev?((cur.rev-prev.rev)/prev.rev*100):(cur.rev?100:0);
  if($("reportCompareKpi")) $("reportCompareKpi").textContent=`${diff>=0?"+":""}${diff.toFixed(1)}%`;
  if($("reportCompareSub")) $("reportCompareSub").textContent=`${monthLabel(cur.month)} ${brl(cur.rev)} x ${monthLabel(prev.month)} ${brl(prev.rev)}`;
  if($("reportSuppliersTable")) $("reportSuppliersTable").innerHTML=supplierStats().slice(0,10).map((s,i)=>`<tr><td>${i+1}</td><td><b>${esc(s.name)}</b></td><td>${s.qty}</td><td>${brl(s.rev)}</td><td class="success">${brl(s.profit)}</td></tr>`).join("") || `<tr><td colspan="5"><small>Sem fornecedores no relatório.</small></td></tr>`;
  if($("reportMarketplaceTable")){const ms=genericStatsFromOrders(selected,orderMarketplace);$("reportMarketplaceTable").innerHTML=ms.map((m,i)=>`<tr><td>${i+1}</td><td><b>${esc(m.name)}</b></td><td>${m.orders}</td><td>${m.qty}</td><td>${brl(m.rev)}</td><td class="success">${brl(m.profit)}</td><td>${brl(m.orders?m.rev/m.orders:0)}</td></tr>`).join("") || `<tr><td colspan="7"><small>Sem marketplace para o período.</small></td></tr>`;}
  if($("reportStatesTable")){const st=genericStatsFromOrders(selected,o=>String(o.customerUf||"").toUpperCase()).slice(0,10);$("reportStatesTable").innerHTML=st.map((m,i)=>`<tr><td>${i+1}</td><td><b>${esc(m.name)}</b></td><td>${m.orders}</td><td>${m.qty}</td><td>${brl(m.rev)}</td><td class="success">${brl(m.profit)}</td></tr>`).join("") || `<tr><td colspan="6"><small>Sem estados para o período.</small></td></tr>`;}
  if($("reportClientsTable")){const cs=genericStatsFromOrders(selected,o=>o.customerName||"Cliente sem nome").slice(0,10);$("reportClientsTable").innerHTML=cs.map((m,i)=>`<tr><td>${i+1}</td><td><b>${esc(m.name)}</b></td><td>${m.orders}</td><td>${m.qty}</td><td>${brl(m.rev)}</td><td class="success">${brl(m.profit)}</td><td>${m.last||"-"}</td></tr>`).join("") || `<tr><td colspan="7"><small>Sem clientes para o período.</small></td></tr>`;}
  if($("reportCompareTable")){const cmp=[cur,prev];$("reportCompareTable").innerHTML=cmp.map(m=>`<tr><td><b>${monthLabel(m.month)}</b></td><td>${m.orders}</td><td>${m.qty}</td><td>${brl(m.rev)}</td><td class="success">${brl(m.profit)}</td><td>${brl(m.orders?m.rev/m.orders:0)}</td></tr>`).join("");}
  drawMonthlyBars(months);
}
...
function normalizeImportData(d){
  ...
  const rawQty=Number(firstDefined(d.quantity,d.quantidade,d.qtd,d.quantidadeVendida,d.quantidade_vendida,product.quantity,sale.quantity,1))||1;
  const finalSaleShipping=saleShipping || (totalRevenue && salePrice ? totalRevenue-(salePrice*rawQty) : 0);
  return {
...
function updateAiCalc(){
  const qty=Math.max(1,parseInt($("aiQuantity")?.value||"1",10)||1);
  const revenue=(num($("aiSalePrice")?.value)*qty)+num($("aiSaleShipping")?.value);
  const supplier=(num($("aiBuyPrice")?.value)*qty)+num($("aiBuyShipping")?.value)-num($("aiBuyDiscount")?.value);
...
function setupAi(){
  ...
  ["aiSalePrice","aiQuantity","aiSaleShipping","aiBuyPrice","aiBuyShipping","aiBuyDiscount","aiAmazonFees"].forEach(id=>{ if($(id)) $(id).addEventListener("input",updateAiCalc); });
...
function saveAiOrder(){const d=aiData();if(!d.productName||!d.customerName)return alert("Produto e cliente são obrigatórios.");const p=findOrCreateProduct(d);let o={id:uuid(),orderDate:d.orderDate||today(),amazonOrderId:d.amazonOrderId,productId:p.id,productName:p.name,customerName:d.customerName,customerPhone:d.customerPhone,customerCep:d.customerCep,customerAddress:d.customerAddress,customerNumber:d.customerNumber,customerComplement:d.customerComplement,customerDistrict:d.customerDistrict,customerCity:d.customerCity,customerUf:d.customerUf,supplierId:d.supplierId,buyLink:d.buyLink,status:d.status||"Venda realizada Amazon",salePrice:d.salePrice,quantity:d.quantity||1,saleShipping:d.saleShipping,buyPrice:d.buyPrice,buyShipping:d.buyShipping,buyDiscount:d.buyDiscount,amazonFees:d.amazonFees,trackingCode:d.trackingCode,trackingSent:d.trackingSent,messageTemplateId:d.messageTemplateId,notes:d.notes,createdAt:new Date().toISOString()};let orders=read(K.orders);if(!confirmIfDuplicate(o,orders,o.id)) return;o.customerId=upsertCustomer(o);orders.push(o);write(K.orders,orders);logAiImport(d,o.id,p.id);render();alert("Pedido importado com sucesso.");openView("dashboard");}
function sendAiToOrder(){const d=aiData();const p=findOrCreateProduct(d);render();$("orderProductId").value=p.id;$("orderDate").value=d.orderDate||"";$("amazonOrderId").value=d.amazonOrderId;$("customerName").value=d.customerName;$("customerPhone").value=d.customerPhone;$("customerCep").value=d.customerCep;$("customerAddress").value=d.customerAddress;$("customerNumber").value=d.customerNumber;$("customerComplement").value=d.customerComplement;$("customerDistrict").value=d.customerDistrict;$("customerCity").value=d.customerCity;$("customerUf").value=d.customerUf;$("orderSupplierId").value=d.supplierId;$("buyLink").value=d.buyLink;$("orderStatus").value=d.status||"Venda realizada Amazon";$("salePrice").value=d.salePrice;if($("orderQuantity")) $("orderQuantity").value=d.quantity||1;$("saleShipping").value=d.saleShipping;$("buyPrice").value=d.buyPrice;$("buyShipping").value=d.buyShipping;$("buyDiscount").value=d.buyDiscount;$("amazonFees").value=d.amazonFees;$("trackingCode").value=d.trackingCode;$("trackingSent").value=d.trackingSent;$("messageTemplateId").value=d.messageTemplateId;$("orderNotes").value=d.notes;openView("orders");}

function smartOpenFromGlobalSearch(){
  const qRaw=String($("globalSearch")?.value||"").trim();
  const q=cleanKey(qRaw);
  if(!q) return false;
  const view=activeView();
  if(view==="orders" || /^\d{3}[- ]?\d{7}[- ]?\d{7}/.test(qRaw)){
    const found=findOrderByAmazonCode(qRaw) || filteredOrders()[0];
    if(found){ editOrder(found.id); setSyncStatus(`Pedido ${found.amazonOrderId||qRaw} carregado no formulário.`, 'ok'); return true; }
  }
  if(view==="products"){
    const rows=sortByName(read(K.products).filter(p=>textMatch(`${p.name} ${p.category||""} ${p.asin||""} ${p.sku||""} ${supplierName(p.supplierId)} ${p.status||""} ${p.notes||""}`,q)));
    const target=rows.find(p=>[p.name,p.asin,p.sku].some(v=>cleanKey(v)===q)) || rows[0];
    if(target){ fillProductForm(target,"search"); document.querySelector("#products .formPanel, #productForm")?.scrollIntoView({behavior:"smooth",block:"start"}); return true; }
  }
  if(view==="customers"){
    const rows=read(K.customers).filter(c=>textMatch(`${c.name} ${c.phone||""} ${c.cep||""} ${c.address||""} ${c.city||""} ${c.uf||""}`,q));
    if(rows[0]){ openCustomerEdit(rows[0].id); return true; }
  }
  if(view==="suppliers"){
    const rows=read(K.suppliers).filter(sp=>textMatch(`${sp.name} ${sp.type||""} ${sp.whatsapp||""} ${sp.email||""}`,q));
    if(rows[0]){ editSupplier(rows[0].id); return true; }
  }
  return false;
}
...
function updateOrderCalcPreview(){
  ...
  const d={salePrice:num($("salePrice").value),quantity:Math.max(1,parseInt($("orderQuantity")?.value||"1",10)||1),saleShipping:num($("saleShipping").value),buyPrice:num($("buyPrice").value),buyShipping:num($("buyShipping").value),buyDiscount:num($("buyDiscount").value),amazonFees:num($("amazonFees").value)};
  $("orderCalcPreview").innerHTML=`<span class="calcLabel">Receita</span> <span class="calcValue">${brl(revenue(d))}</span> • <span class="calcLabel">Fornecedor</span> <span class="calcValue">${brl(supplierOrderCost(d))}</span> • <span class="calcLabel">Amazon</span> <span class="calcAmazon">${brl(d.amazonFees)}</span> • <span class="calcLabel">Itens</span> <span class="calcValue">${lineQuantity(d)}</span> • <span class="calcLabel">Lucro líquido</span> <span class="calcProfit">${brl(profit(d))}</span> • <span class="calcLabel">Margem</span> <span class="calcMargin">${margin(profit(d),revenue(d)).toFixed(1)}%</span>`;
}
...
function render(){
  renderSelects(); renderPurchaseAccounts(); renderDashboard(); renderAmazonMetrics(); renderOrdersTable(); renderProducts(); renderCustomers(); renderSuppliers(); renderMessages(); renderFinance(); renderReports(); renderAnalytics(); renderSettingsStats(); if(typeof v20RenderAgents==="function") v20RenderAgents(); updateOrderCalcPreview(); updateProductCalcPreview();
}
...
// V20 Beta — Central de Agentes
const NPC_V20_DECISIONS_KEY=K.agentDecisions;
const NPC_V20_LOG_KEY=K.agentLog;
const NPC_V20_CONFIG_KEY=K.agentConfig;
function v20ReadJson(key,fallback=[]){try{return JSON.parse(localStorage.getItem(key)||JSON.stringify(fallback));}catch(e){return fallback;}}
function v20WriteJson(key,value){localStorage.setItem(key,JSON.stringify(value));}
function v20Log(type,message,details={}){const rows=v20ReadJson(NPC_V20_LOG_KEY,[]);rows.unshift({id:uuid(),type,message,details,at:new Date().toISOString()});v20WriteJson(NPC_V20_LOG_KEY,rows.slice(0,100));}
function v20SupplierOptionsForOrder(o){
  const p=read(K.products).find(x=>x.id===o.productId)||{};
  const suppliers=read(K.suppliers);
  const options=[];
  if(p.supplierId || num(p.buyPrice)>0){const sup=suppliers.find(x=>x.id===p.supplierId);options.push({source:'catalog',supplierId:p.supplierId||'',name:sup?.name||'Fornecedor padrão',price:num(p.buyPrice),shipping:num(p.buyShipping),url:p.buyLink||'',deadline:sup?.leadTime||''});}
  if(o.supplierId || num(o.buyPrice)>0){const sup=suppliers.find(x=>x.id===o.supplierId);options.push({source:'order',supplierId:o.supplierId||'',name:sup?.name||'Fornecedor do pedido',price:num(o.buyPrice),shipping:num(o.buyShipping),url:o.buyLink||'',deadline:sup?.leadTime||''});}
  if(typeof v194ReadLeads==='function') v194ReadLeads().filter(l=>l.productId===o.productId).forEach(l=>options.push({source:'radar',supplierId:'',leadId:l.id,name:l.name,price:num(l.price),shipping:num(l.shipping),url:l.url||'',deadline:l.deadline||''}));
  const seen=new Set();return options.filter(x=>{const k=`${cleanKey(x.name)}|${x.price}|${x.shipping}`;if(seen.has(k))return false;seen.add(k);return x.price+x.shipping>0;}).map(x=>({...x,total:x.price+x.shipping})).sort((a,b)=>a.total-b.total);
}
function v20BuildDecision(o){const opts=v20SupplierOptionsForOrder(o);if(!opts.length)return null;const best=opts[0], current=supplierOrderCost(o)||opts[opts.length-1].total;return {id:o.id,orderId:o.id,amazonOrderId:o.amazonOrderId||'',productId:o.productId||'',productName:o.productName||'',saleRevenue:revenue(o),bestSupplier:best.name,bestSupplierId:best.supplierId||'',bestLeadId:best.leadId||'',bestCost:best.total,bestUrl:best.url||'',saving:Math.max(0,current-best.total),expectedProfit:revenue(o)-num(o.amazonFees)-best.total,optionCount:opts.length,status:'pending',createdAt:new Date().toISOString()};}
function v20RunComparator(){
  const orders=read(K.orders).filter(o=>!['Entregue','Cancelado','Devolvido','Reembolsado'].includes(o.status));
  const current=v20ReadJson(NPC_V20_DECISIONS_KEY,[]);const byId=new Map(current.map(d=>[d.orderId,d]));let compared=0;
  orders.forEach(o=>{const fresh=v20BuildDecision(o);if(!fresh)return;const old=byId.get(o.id);byId.set(o.id,old?.status==='approved'?{...fresh,...old,updatedAt:new Date().toISOString()}:{...fresh,status:old?.status||'pending'});compared++;});
  v20WriteJson(NPC_V20_DECISIONS_KEY,[...byId.values()]);v20Log('compare',`Comparador analisou ${compared} pedido(s).`);return compared;
}
async function v20FetchAmazonOrders(){
  const endpoint=$('amazonAgentEndpoint')?.value.trim()||v20ReadJson(NPC_V20_CONFIG_KEY,{}).endpoint||'';
  if(!endpoint) throw new Error('Endpoint backend não configurado. A SP-API não deve ser chamada diretamente pelo navegador.');
  const res=await fetch(endpoint,{headers:{'Accept':'application/json'}});if(!res.ok)throw new Error(`Backend respondeu HTTP ${res.status}`);const data=await res.json();return Array.isArray(data)?data:(data.orders||[]);
}
function v20ImportOrderPayload(d){
  if(!d.amazonOrderId) throw new Error('amazonOrderId é obrigatório.');
  let arr=read(K.orders);if(arr.some(o=>normalizeOrderId(o.amazonOrderId)===normalizeOrderId(d.amazonOrderId)))return false;
  let product=read(K.products).find(p=>cleanKey(p.sku)===cleanKey(d.sku)||cleanKey(p.asin)===cleanKey(d.asin)||cleanKey(p.name)===cleanKey(d.productName));
  if(!product){product={id:uuid(),name:d.productName||d.title||'Produto importado pela Amazon',asin:d.asin||'',sku:d.sku||'',category:'',supplierId:'',buyLink:'',status:'Ativo na Amazon',buyPrice:0,buyShipping:0,salePrice:num(d.salePrice),saleShipping:num(d.saleShipping),amazonFees:num(d.amazonFees),notes:'Criado pelo Agente de Novas Vendas v20 Beta'};const products=read(K.products);products.push(product);write(K.products,products);}
  const o={id:uuid(),orderDate:d.orderDate||d.purchaseDate?.slice?.(0,10)||today(),amazonOrderId:d.amazonOrderId,productId:product.id,productName:product.name,customerName:d.customerName||'Cliente Amazon',customerPhone:d.customerPhone||'',customerCep:d.customerCep||'',customerAddress:d.customerAddress||'',customerNumber:d.customerNumber||'',customerComplement:d.customerComplement||'',customerDistrict:d.customerDistrict||'',customerCity:d.customerCity||'',customerUf:d.customerUf||'',supplierId:product.supplierId||'',purchaseMarketplace:'',marketplaceOrderId:'',purchaseAccount:'',buyLink:product.buyLink||'',status:'Venda realizada Amazon',salePrice:num(d.salePrice||d.itemPrice),quantity:Math.max(1,num(d.quantity)||1),saleShipping:num(d.saleShipping),buyPrice:num(product.buyPrice),buyShipping:num(product.buyShipping),buyDiscount:0,amazonFees:num(d.amazonFees||product.amazonFees),trackingCode:'',trackingSent:'Não',notes:'Importado pelo Agente de Novas Vendas v20 Beta',createdAt:new Date().toISOString()};o.customerId=upsertCustomer(o);arr.push(o);write(K.orders,arr);v20Log('sale',`Nova venda identificada: ${o.amazonOrderId}.`,{orderId:o.id});return true;
}
async function v20RunSalesAgent(){const orders=await v20FetchAmazonOrders();let imported=0;orders.forEach(d=>{if(v20ImportOrderPayload(d))imported++;});v20Log('sale',`Agente Amazon consultou ${orders.length} registro(s) e importou ${imported} venda(s).`);return imported;}
function v20ApproveDecision(orderId){
  const decisions=v20ReadJson(NPC_V20_DECISIONS_KEY,[]);const d=decisions.find(x=>x.orderId===orderId);if(!d)return;
  const orders=read(K.orders);const ix=orders.findIndex(o=>o.id===orderId);if(ix<0)return;
  const o=orders[ix];orders[ix]={...o,supplierId:d.bestSupplierId||o.supplierId,buyPrice:d.bestCost,buyShipping:0,buyLink:d.bestUrl||o.buyLink,agentDecision:'approved',updatedAt:new Date().toISOString()};write(K.orders,orders);
  d.status='approved';d.approvedAt=new Date().toISOString();v20WriteJson(NPC_V20_DECISIONS_KEY,decisions);v20Log('decision',`Compra aprovada para ${d.amazonOrderId||d.productName}: ${d.bestSupplier}.`);render();
}
function v20OpenDecision(orderId){const d=v20ReadJson(NPC_V20_DECISIONS_KEY,[]).find(x=>x.orderId===orderId);if(d?.bestUrl)window.open(d.bestUrl,'_blank');else alert('O fornecedor escolhido não possui link cadastrado.');}
function v20EditPurchase(orderId){openView('orders');setTimeout(()=>editOrder(orderId),0);}
function v20RenderAgents(){
  if(!$('agentDecisionTable'))return;const decisions=v20ReadJson(NPC_V20_DECISIONS_KEY,[]);const filter=$('agentDecisionFilter')?.value||'pending';const rows=decisions.filter(d=>filter==='all'||d.status===filter);
  const pending=decisions.filter(d=>d.status==='pending').length,approved=decisions.filter(d=>d.status==='approved').length;
  if($('agentSalesCount'))$('agentSalesCount').textContent=read(K.orders).filter(o=>String(o.notes||'').includes('Agente de Novas Vendas')).length;
  if($('agentCompareCount'))$('agentCompareCount').textContent=decisions.length;if($('agentDecisionCount'))$('agentDecisionCount').textContent=pending;if($('agentApprovedCount'))$('agentApprovedCount').textContent=approved;
  const cfg=v20ReadJson(NPC_V20_CONFIG_KEY,{});if($('amazonAgentEndpoint')&&!$('amazonAgentEndpoint').value)$('amazonAgentEndpoint').value=cfg.endpoint||'';if($('amazonAgentInterval'))$('amazonAgentInterval').value=cfg.interval||'5';if($('agentSalesStatus'))$('agentSalesStatus').textContent=(cfg.endpoint||$('amazonAgentEndpoint')?.value)?'Endpoint configurado':'SP-API não configurada';
  $('agentDecisionTable').innerHTML=rows.map(d=>`<tr><td><b>${esc(d.amazonOrderId||'-')}</b></td><td>${esc(d.productName)}</td><td>${brl(d.saleRevenue)}</td><td><b>${esc(d.bestSupplier)}</b><br><small>${d.optionCount} opção(ões)</small></td><td>${brl(d.bestCost)}</td><td class="success">${brl(d.saving)}</td><td class="success">${brl(d.expectedProfit)}</td><td><span class="status ${d.status==='approved'?'done':'pending'}">${d.status==='approved'?'Aprovada':'Pendente'}</span></td><td><div class="actionGroup"><button onclick="v20OpenDecision('${d.orderId}')">Abrir fornecedor</button>${d.status!=='approved'?`<button class="primary" onclick="v20ApproveDecision('${d.orderId}')">Aprovar</button>`:''}<button onclick="v20EditPurchase('${d.orderId}')">Registrar compra</button></div></td></tr>`).join('')||'<tr><td colspan="9"><small>Nenhuma decisão nesta fila. Execute os agentes ou cadastre opções no Radar de Fornecedores.</small></td></tr>';
  const logs=v20ReadJson(NPC_V20_LOG_KEY,[]);if($('agentLog'))$('agentLog').innerHTML=logs.map(l=>`<div><b>${esc(l.message)}</b><small>${new Date(l.at).toLocaleString('pt-BR')}</small></div>`).join('')||'<p class="muted">Nenhuma execução registrada.</p>';
}
function v20SetupAgents(){
  if($('runAgentsBtn'))$('runAgentsBtn').onclick=async()=>{try{const cfg={endpoint:$('amazonAgentEndpoint')?.value.trim()||'',interval:$('amazonAgentInterval')?.value||'5'};v20WriteJson(NPC_V20_CONFIG_KEY,cfg);let imported=0;if(cfg.endpoint)imported=await v20RunSalesAgent();const compared=v20RunComparator();render();alert(`Agentes concluídos. Novas vendas: ${imported}. Comparações: ${compared}.`);}catch(e){v20Log('error',e.message);v20RunComparator();render();alert(e.message);}};
  if($('testAmazonAgentBtn'))$('testAmazonAgentBtn').onclick=async()=>{try{const rows=await v20FetchAmazonOrders();alert(`Conexão concluída. ${rows.length} pedido(s) recebido(s) do backend.`);}catch(e){alert(e.message);}};
  if($('importAgentOrderBtn'))$('importAgentOrderBtn').onclick=()=>{try{const raw=$('agentOrderJson').value.trim();if(!raw)throw new Error('Cole um JSON de venda para teste.');const ok=v20ImportOrderPayload(JSON.parse(raw));v20RunComparator();render();alert(ok?'Venda de teste importada.':'Este pedido já existe.');}catch(e){alert(`JSON inválido: ${e.message}`);}};
  if($('agentDecisionFilter'))$('agentDecisionFilter').onchange=v20RenderAgents;
  if($('clearAgentLogBtn'))$('clearAgentLogBtn').onclick=()=>{v20WriteJson(NPC_V20_LOG_KEY,[]);v20RenderAgents();};
  ['amazonAgentEndpoint','amazonAgentInterval'].forEach(id=>{if($(id))$(id).onchange=()=>v20WriteJson(NPC_V20_CONFIG_KEY,{endpoint:$('amazonAgentEndpoint')?.value.trim()||'',interval:$('amazonAgentInterval')?.value||'5'});});
}
window.v20ApproveDecision=v20ApproveDecision;window.v20OpenDecision=v20OpenDecision;window.v20EditPurchase=v20EditPurchase;
setTimeout(()=>{v20SetupAgents();v20RenderAgents();},300);
...
// ================= V21 BETA · AMAZON CONNECTED EDITION =================
const NPC_V21_AMAZON_CFG=K.amazonIntegration;
const NPC_V21_AMAZON_CATALOG=K.amazonCatalog;
const NPC_V21_AMAZON_LOG=K.amazonLog;
function v21Read(key,fallback){try{return JSON.parse(localStorage.getItem(key)||JSON.stringify(fallback));}catch(e){return fallback;}}
function v21Write(key,value){localStorage.setItem(key,JSON.stringify(value));}
function v21AmazonConfig(){return {...{baseUrl:location.origin==='null'?'':location.origin,marketplaceId:'A2Q3Y263D00KWC',autoSync:'off',importMode:'update'},...v21Read(NPC_V21_AMAZON_CFG,{})};}
function v21Api(path){const cfg=v21AmazonConfig();const base=(cfg.baseUrl||'').replace(/\/$/,'');if(!base)throw new Error('Informe a URL do backend da V21.');return `${base}/api/amazon/${path}`;}
function v21AmazonLog(message,type='info'){const logs=v21Read(NPC_V21_AMAZON_LOG,[]);logs.unshift({message,type,at:new Date().toISOString()});v21Write(NPC_V21_AMAZON_LOG,logs.slice(0,100));v21RenderAmazonLog();}
function v21RenderAmazonLog(){const el=$('amazonIntegrationLog');if(!el)return;const logs=v21Read(NPC_V21_AMAZON_LOG,[]);el.innerHTML=logs.map(l=>`<div><b>${esc(l.message)}</b><small>${new Date(l.at).toLocaleString('pt-BR')}</small></div>`).join('')||'<p class="muted">Nenhuma atividade registrada.</p>';}
function v21NormalizeListing(x){const summary=(x.summaries||[])[0]||{};const offer=(x.offers||[])[0]||{};const price=offer.price?.amount||offer.price?.value||x.price||0;return {sku:x.sku||x.sellerSku||'',asin:summary.asin||x.asin||x.identifiers?.marketplaceAsin?.asin||'',title:summary.itemName||summary.title||x.title||x.productName||'Produto Amazon',status:(x.issues?.length?'Com pendências':(x.status||x.fulfillmentAvailability?.[0]?.fulfillmentChannelCode||'Ativo')),price:num(price),raw:x};}
function v21FindLocalProduct(item){return read(K.products).find(p=>(item.sku&&cleanKey(p.sku)===cleanKey(item.sku))||(item.asin&&cleanKey(p.asin)===cleanKey(item.asin)));}
function v21RenderCatalog(){const rows=v21Read(NPC_V21_AMAZON_CATALOG,[]);if($('amazonCatalogCount'))$('amazonCatalogCount').textContent=rows.length;if($('amazonLinkedCount'))$('amazonLinkedCount').textContent=read(K.products).filter(p=>p.asin||p.sku).length;const table=$('amazonCatalogTable');if(!table)return;table.innerHTML=rows.map((x,i)=>{const local=v21FindLocalProduct(x);return `<tr><td><b>${esc(x.title)}</b></td><td>${esc(x.asin||'-')}</td><td>${esc(x.sku||'-')}</td><td>${brl(x.price)}</td><td><span class="status">${esc(x.status||'Ativo')}</span></td><td>${local?`<span class="status done">${esc(local.name)}</span>`:'<span class="status pending">Não vinculado</span>'}</td><td><button onclick="v21ImportOne(${i})">${local?'Atualizar':'Importar'}</button></td></tr>`}).join('')||'<tr><td colspan="7"><small>Nenhum produto retornado.</small></td></tr>';}
function v21SaveConfig(){const cfg={baseUrl:$('amazonBackendBase')?.value.trim()||'',marketplaceId:$('amazonMarketplace')?.value||'A2Q3Y263D00KWC',autoSync:$('amazonAutoSync')?.value||'off',importMode:$('amazonImportMode')?.value||'update'};v21Write(NPC_V21_AMAZON_CFG,cfg);const agent=v20ReadJson(NPC_V20_CONFIG_KEY,{});v20WriteJson(NPC_V20_CONFIG_KEY,{...agent,endpoint:cfg.baseUrl?`${cfg.baseUrl.replace(/\/$/,'')}/api/amazon/orders`:agent.endpoint});return cfg;}
async function v21CheckConnection(showAlert=false){try{v21SaveConfig();const res=await fetch(v21Api('status'),{credentials:'include'});const data=await res.json();if(!res.ok)throw new Error(data.error||`HTTP ${res.status}`);const connected=!!data.connected;if($('amazonConnectionBadge')){$('amazonConnectionBadge').textContent=connected?'Conectado':'Não conectado';$('amazonConnectionBadge').className=`status ${connected?'done':'pending'}`;}if($('amazonConnectionKpi'))$('amazonConnectionKpi').textContent=connected?'Online':'Offline';if($('amazonConnectionDetail'))$('amazonConnectionDetail').textContent=data.sellerId?`Seller ${data.sellerId}`:(data.message||'Autorização necessária');if(showAlert)alert(connected?'Conta Amazon conectada.':'Backend disponível, mas a conta ainda não foi autorizada.');return connected;}catch(e){if($('amazonConnectionDetail'))$('amazonConnectionDetail').textContent=e.message;if(showAlert)alert(`Não foi possível verificar: ${e.message}`);return false;}}
async function v21LoadCatalog(){v21SaveConfig();const res=await fetch(`${v21Api('catalog')}?marketplaceId=${encodeURIComponent(v21AmazonConfig().marketplaceId)}`,{credentials:'include'});const data=await res.json();if(!res.ok)throw new Error(data.error||`HTTP ${res.status}`);const items=(data.items||data.listings||data).map(v21NormalizeListing);v21Write(NPC_V21_AMAZON_CATALOG,items);const cfg=v21AmazonConfig();cfg.lastSync=new Date().toISOString();v21Write(NPC_V21_AMAZON_CFG,cfg);v21AmazonLog(`Catálogo consultado: ${items.length} produto(s) recebido(s).`);v21RenderAmazon();return items;}
function v21UpsertListing(item){let products=read(K.products);let p=v21FindLocalProduct(item);if(p){p.name=item.title||p.name;p.asin=item.asin||p.asin;p.sku=item.sku||p.sku;p.salePrice=item.price||p.salePrice;p.status=item.status==='Com pendências'?'Pausado':'Ativo na Amazon';p.amazonLastSync=new Date().toISOString();}else{p={id:uuid(),name:item.title,category:'',asin:item.asin,sku:item.sku,supplierId:'',buyLink:'',status:'Ativo na Amazon',buyPrice:0,buyShipping:0,salePrice:item.price,saleShipping:0,amazonFees:0,notes:'Importado do catálogo Amazon pela V21 Beta',amazonLastSync:new Date().toISOString()};products.push(p);}write(K.products,products);return p;}
function v21ImportOne(index){const item=v21Read(NPC_V21_AMAZON_CATALOG,[])[index];if(!item)return;const p=v21UpsertListing(item);v21AmazonLog(`Produto sincronizado: ${p.name} · ASIN ${p.asin||'-'} · SKU ${p.sku||'-'}.`);render();v21RenderAmazon();}
function v21SyncAll(){const cfg=v21AmazonConfig(),items=v21Read(NPC_V21_AMAZON_CATALOG,[]);if(cfg.importMode==='preview'){alert('O modo atual é apenas visualizar. Altere para importar nas configurações.');return;}let created=0,updated=0;items.forEach(item=>{const exists=!!v21FindLocalProduct(item);if(cfg.importMode==='new'&&exists)return;v21UpsertListing(item);exists?updated++:created++;});v21AmazonLog(`Sincronização concluída: ${created} criado(s) e ${updated} atualizado(s).`);render();v21RenderAmazon();alert(`Catálogo sincronizado. Novos: ${created}. Atualizados: ${updated}.`);}
function v21RenderAmazon(){const cfg=v21AmazonConfig();if($('amazonBackendBase')&&!$('amazonBackendBase').value)$('amazonBackendBase').value=cfg.baseUrl||'';if($('amazonMarketplace'))$('amazonMarketplace').value=cfg.marketplaceId;if($('amazonAutoSync'))$('amazonAutoSync').value=cfg.autoSync;if($('amazonImportMode'))$('amazonImportMode').value=cfg.importMode;if($('amazonLastSync'))$('amazonLastSync').textContent=cfg.lastSync?new Date(cfg.lastSync).toLocaleDateString('pt-BR'):'Nunca';if($('amazonSyncMessage'))$('amazonSyncMessage').textContent=cfg.lastSync?new Date(cfg.lastSync).toLocaleString('pt-BR'):'Nenhuma sincronização realizada';v21RenderCatalog();v21RenderAmazonLog();}
function v21SetupAmazon(){if(!$('amazonConnectBtn'))return;v21RenderAmazon();$('amazonConnectBtn').onclick=()=>{try{v21SaveConfig();location.href=v21Api('auth/start');}catch(e){alert(e.message);}};$('amazonCheckBtn').onclick=()=>v21CheckConnection(true);$('amazonDisconnectBtn').onclick=async()=>{try{await fetch(v21Api('disconnect'),{method:'POST',credentials:'include'});v21AmazonLog('Conexão Amazon removida deste navegador.');v21CheckConnection(false);}catch(e){alert(e.message);}};$('amazonLoadCatalogBtn').onclick=async()=>{try{$('amazonLoadCatalogBtn').disabled=true;await v21LoadCatalog();}catch(e){v21AmazonLog(e.message,'error');alert(e.message);}finally{$('amazonLoadCatalogBtn').disabled=false;}};$('amazonSyncCatalogBtn').onclick=v21SyncAll;$('amazonClearLogBtn').onclick=()=>{v21Write(NPC_V21_AMAZON_LOG,[]);v21RenderAmazonLog();};['amazonBackendBase','amazonMarketplace','amazonAutoSync','amazonImportMode'].forEach(id=>$(id).onchange=v21SaveConfig);v21CheckConnection(false);}
window.v21ImportOne=v21ImportOne;
setTimeout(v21SetupAmazon,350);
